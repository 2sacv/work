...
function ResetTable()
{
    ...
    for (var i = 0; i < usersInfo.length; i++)
    {
        insert += '<tr align="center" style="height:25px;background-color:#2C2C2C">';
    ...
    if(usersInfo.length>0){
     ...
     $("#userList tbody tr").click(function(){
            $(this).css("background-color", "#555555").siblings().css("background-color", "#2C2C2C");
...
function CheckPassword(szPassword)
{
    ...
    if (szPassword.length < 4 || 16 < szPassword.getBytes())
    {
        alert(IDC_PASSWORD_LENGTH);
        return false;
    }
...
function ModifyUsr()
{
    ...
    $("#userList").find("tbody>tr").each(function(i){
        ...
        if ("#555555" == bc || (bc.indexOf("#")==-1 && "#555555" == RGB2HEX(bc)))
        {
            index = i;
        }
    });
    ...
    if(usersInfo[index][0].toString() != user){
        alert(IDC_USRPASSWD_NOT_MODIFY);
        window.focus();
        return false;
    }
