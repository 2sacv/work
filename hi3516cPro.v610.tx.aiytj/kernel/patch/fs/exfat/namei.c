...
static bool is_entry_isolated(struct super_block *sb,
			      struct exfat_chain *p_dir, int entry_idx)
{
	...
	/* 前一项是 FILE 或 DIR → 当前项可能是其扩展，不安全 */
	if (entry_idx > 0) {
		ep = exfat_get_dentry(sb, p_dir, entry_idx - 1, &bh, &sector);
		if (ep) {
	...
	/* 后一项是 STREAM 或 EXTEND → 当前项可能是文件首项，不安全 */
	ep = exfat_get_dentry(sb, p_dir, entry_idx + 1, &bh, &sector);
	if (ep) {
		type = exfat_get_entry_type(ep);
		brelse(bh);
		if (type == TYPE_STREAM || type == TYPE_EXTEND)
			return false;
	}
		...
		for (; i < dentries_per_clu; i++, dentry++) {
			...
			if (type == TYPE_UNUSED || type == TYPE_DELETED) {
				...
				if (type == TYPE_UNUSED &&
				 ...
				 if ((hint_femp->eidx != EXFAT_HINT_NONE) &&
					(hint_femp->count == CNT_UNUSED_HIT)) {
					/* Safety: only fix when isolated */
					if (is_entry_isolated(sb, &clu, i)) {
						struct exfat_dentry *fix_ep;
						struct buffer_head *fix_bh;
						sector_t fix_sector;

						pr_warn_ratelimited("exFAT-fs (%s): "
							"found bogus dentry(%d) "
							"beyond unused empty group(%d) "
							"(start_clu : %u, cur_clu : %u) - fix it\n",
							sb->s_id,
							dentry, hint_femp->eidx, p_dir->dir,
							clu.dir);

						fix_ep = exfat_get_dentry(sb, &clu, i,
							&fix_bh, &fix_sector);
						if (fix_ep) {
							fix_ep->type = EXFAT_UNUSED;
							mark_buffer_dirty(fix_bh);
							sync_dirty_buffer(fix_bh);
							brelse(fix_bh);
							num_empty++;
