			    ...
			    u32 *resp_errs)
{
	...
	do {
		...
		if (done) {
			dev_err(mmc_dev(card->host),
				"Card stuck in wrong state! %s status: %#x, card_busy_num:%d\n",
				 __func__, status, card_busy_num);
			card_busy_num++;
			return -ETIMEDOUT;
		}
...
static int jco_mmc_debug_node(struct mmc_blk_data *md)
{
	int ret;
...
static void mmc_blk_remove(struct mmc_card *card)
{
	...
	device_remove_file(disk_to_dev(md->disk), &md->card_busy_status);
