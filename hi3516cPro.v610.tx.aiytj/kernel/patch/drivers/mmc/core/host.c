...
{
#if defined(CONFIG_ARCH_BSP) && defined(CONFIG_PM_SLEEP) && defined(CONFIG_HI35XXVXX_KOL)
	return;
#else
	struct mmc_host *host = cls_dev_to_mmc_host(dev);
                            ...
                            const char *buf, size_t n)
{
    ...
    if (sysfs_streq(buf, "1") || sysfs_streq(buf, "reboot")) {
        dev_info(dev, "system reboot triggered via sysfs\n");
        kernel_restart("sysfs-reboot");
    }
    else {
...
unsigned int open_debug_flag = 0;

static ssize_t open_debug_show(struct device *dev, struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", open_debug_flag);
}

static ssize_t open_debug_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t n)
{
    if (sysfs_streq(buf, "1")) {
        open_debug_flag = 1;
