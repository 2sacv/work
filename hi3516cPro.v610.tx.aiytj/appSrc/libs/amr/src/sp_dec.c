 ...
 *    void
 */
static int Decoder_amr_reset(Decoder_amrState *state, enum Mode mode)
{
    Word32  i;
                    ...
                    Word32 synth[], Word32 A_t[])
{
    ...
    for (i = 0; i < M; i++) {
        lsf_int_variab[i] =
            lsf_int_variab[i] +
            ((lsf_variab_factor * st->lsf_hist_mean[i + lsf_variab_index * M]) >> 15);
    }
                     ...
                     Word32 * lsp2_q)
{
    ...
    if (bfi != 0) {
        ...
        for (i = 0; i < M; i += 2) {
            ...
            temp2 =
                mean_lsf_5[i + 1] + ((st->past_r_q[i + 1] * LSP_PRED_FAC_MR122) >> 15);
 ...
 *    void
 */
static void gc_pred_update(gc_predState * st, Word32 qua_ener_MR122, Word32 qua_ener)
{
    Word32  i;
...
static Word32 Cb_gain_average(Cb_gain_averageState * st, enum Mode mode, Word32
                              gain_code, Word32 lsp[], Word32 lspAver[], Word16 bfi,
                              Word16 prev_bf, Word16 pdfi, Word16 prev_pdf,
                              Word32 inBackgroundNoise, Word32 voicedHangover)
{
    Word32  tmp[M];
    ...
    if ((mode <= MR67) | (mode == MR102)) {
        ...
        if ((st->hangCount >= 40) & (diff <= 5325)) {   /* 0.65 in Q13 */
            ...
            if (((((pdfi != 0) & (prev_pdf != 0)) | (bfi != 0) | (prev_bf !=
                                                                  0)) & ((voicedHangover
                                                                          >
                                                                          1)) &
        ...
        if (((bfi != 0) | (prev_bf != 0)) & (inBackgroundNoise != 0) & (mode < MR67)) {
            sum =
                9362 * (st->cbGainHistory[0] + st->cbGainHistory[1] +
                        st->cbGainHistory[2] + st->cbGainHistory[3] +
                        st->cbGainHistory[4] + st->cbGainHistory[5] +
                        st->cbGainHistory[6]);
                      ...
                      Word32 * voicedHangover)
{
     ...
     */
    if ((maxEnergy > LOWERNOISELIMIT) & (currEnergy < FRAMEENERGYLIMIT) & (currEnergy >
                                                                           LOWERNOISELIMIT)
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR) {
        ...
        if (mode != MR122) {
            ...
            if (bfi != 0) {
                ...
                if ((st->inBackgroundNoise != 0) & (st->voicedHangover > 4) & ((mode ==
                                                                                MR475)
                                                                               || (mode
                                                                                   ==
                                                                                   MR515)
        ...
        if (mode == MR475) {
            ...
            if (bfi == 0) {
                Dec_gain(&st->pred_state, mode, index_mr475, code, evenSubfr, &gain_pit,
                         &gain_code);
            } else {
                ec_gain_pitch(&st->ec_gain_p_st, st->state, &gain_pit);
         ...
         */
        if ((st->prev_bf != 0 || bfi != 0) & (st->inBackgroundNoise != 0) & ((mode ==
                                                                              MR475) ||
                                                                             (mode ==
                                                                              MR515) ||
                                                                             (mode ==
        ...
        if (((mode == MR475) || (mode == MR515) ||
             (mode == MR59)) & (st->voicedHangover > 3) & (st->inBackgroundNoise !=
                                                           0) & (bfi != 0)) {
            /*
        ...
        if (((mode == MR475) || (mode == MR515) ||
             (mode == MR59)) & (st->voicedHangover > 5) & (st->inBackgroundNoise !=
                                                           0) & (st->state <
                                                                 4) & (((pdfi != 0) &
                                                                        (st->prev_pdf !=
                                                                         0)) || bfi != 0
        ...
        if (overflow) {
            ...
            Syn_filt_overflow(Az, exc_enhanced, &synth[i_subfr], L_SUBFR, st->mem_syn,
                              1);
        } else {
         ...
         */
        memcpy(&st->old_exc[0], &st->old_exc[L_SUBFR], (PIT_MAX + L_INTERPOL) << 2);
                        ...
                        Word32 * Az_4)
{
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR) {
        ...
        *p1 = *p1 - ((temp2 * st->preemph_state_mem_pre) >> 15);
        ...
        if (overflow) {
            Syn_filt_overflow(Ap4, st->res2, &syn[i_subfr], L_SUBFR, st->mem_syn_pst,
                              1);
            overflow = 0;
        } else {
...
static void Post_Process(Post_ProcessState * st, Word32 signal[])
{
    ...
    do {
        ...
        tmp +=
            (st->y2_hi * -7667) + (((st->y2_lo * (-7667)) & (Word32) 0xffff8000) >> 15);
        tmp += st->x0 * 7699;
        tmp += st->x1 * -15398;
        if (((tmp >> 1) ^ tmp) & mask)
 ...
 *    void
 */
void Speech_Decode_Frame(sSpeechFrame *st, enum Mode mode, Word16 * parm, enum
                         RXFrameType frame_type, Word16 * synth)
{
    ...
    /* Synthesis */
    Decoder_amr(&st->decoder_amrState, mode,
                parm, frame_type, synth_speech, Az_dec);
    Post_Filter(&st->post_state, mode,
                synth_speech, Az_dec);

    /* post HP filter, and 15->16 bits */
    Post_Process(&st->postHP_state, synth_speech);
 ...
 *    success = 0
 */
int Speech_Decode_Frame_init(sSpeechFrame *p_spc_frm)
{
    int ret = -1;
    ...
    do {
        ...
        ret = Decoder_amr_reset(&p_spc_frm->decoder_amrState, MR475);
        if (ret != 0) {
            fprintf(stderr, "[%s:%d] reset failed\n", __FILE__, __LINE__);
            ret = -1;
            break;
        }

        ret = Post_Filter_reset(&p_spc_frm->post_state);
        if (ret != 0) {
            fprintf(stderr, "[%s:%d] reset failed\n", __FILE__, __LINE__);
            ret = -1;
            break;
        ...
        ret = Post_Process_reset(&p_spc_frm->postHP_state);
        if (ret != 0) {
            fprintf(stderr, "[%s:%d] reset failed\n", __FILE__, __LINE__);
            ret = -1;
            break;
