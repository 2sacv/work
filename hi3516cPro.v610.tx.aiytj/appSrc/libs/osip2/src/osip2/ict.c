...
int __osip_ict_init(osip_ict_t ** ict, osip_t * osip, osip_message_t * invite)
{
	...
	*ict = (osip_ict_t *) osip_malloc(sizeof(osip_ict_t));
	...
	/* for INVITE retransmissions */
	{
		...
		if (osip_strncasecmp(proto, "TCP",strlen("TCP")) != 0 && osip_strncasecmp(proto, "TLS",strlen("TLS")) != 0 && osip_strncasecmp(proto, "SCTP",strlen("SCTP")) != 0) {	/* for other reliable protocol than TCP, the timer
