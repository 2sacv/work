...
int __osip_ist_init(osip_ist_t ** ist, osip_t * osip, osip_message_t * invite)
{
	...
	*ist = (osip_ist_t *) osip_malloc(sizeof(osip_ist_t));
	...
	/* for INVITE retransmissions */
	{
		...
		if (osip_strncasecmp(proto, "TCP",strlen("TCP")) != 0 
            && osip_strncasecmp(proto, "TLS",strlen("TLS")) != 0 
            && osip_strncasecmp(proto, "SCTP",strlen("SCTP")) != 0) { /* for other reliable protocol than TCP, the timer
