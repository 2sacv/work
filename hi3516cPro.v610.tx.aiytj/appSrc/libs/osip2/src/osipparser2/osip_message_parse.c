								  ...
								  const char **headers)
{
	...
	*headers = buf;
	...
	/* find the the version and the beginning of headers */
	{
		...
		if (0 != osip_strncasecmp(dest->sip_version, "SIP/2.0",strlen("SIP/2.0"))) {
			OSIP_TRACE(osip_trace
					   (__FILE__, __LINE__, OSIP_ERROR, NULL,
					...
					const char **next_body, size_t length)
{
	...
	if (0 != osip_strncasecmp(sip->content_type->type, "multipart",strlen("multipart"))) {
		...
		if (sip->content_length != NULL)
			...
			if (0 == osip_strncasecmp(sip->content_type->type, "application",strlen("application"))
				&& 0 == osip_strncasecmp(sip->content_type->subtype, "sdp",strlen("sdp"))) {
