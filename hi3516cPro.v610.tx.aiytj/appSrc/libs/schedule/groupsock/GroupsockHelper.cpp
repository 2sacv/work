...
char const* timestampString()
{
    ...
    ctime_r(&tvNow.tv_sec, timebuf);
    
    char const* from = &timebuf[11];
    int i;
    for (i = 0; i < 8; ++i) {
        timeString[i] = from[i];
    }
...
// begin -- add by tangpengcheng on 2014/04/01
unsigned setKeepAliveTime(UsageEnvironment& env, int socket, unsigned requestedtime)
{
    int keepalive = 1;
    int keepinterval = 5;
    int keepcount = 3;

    int iret = setsockopt(socket, SOL_SOCKET, SO_KEEPALIVE, (void *)&keepalive , sizeof(keepalive ));
    iret += setsockopt(socket, IPPROTO_TCP, TCP_KEEPIDLE, (void*)&requestedtime , sizeof(requestedtime ));
    iret += setsockopt(socket, IPPROTO_TCP, TCP_KEEPINTVL, (void *)&keepinterval , sizeof(keepinterval ));
    iret += setsockopt(socket, IPPROTO_TCP, TCP_KEEPCNT, (void *)&keepcount , sizeof(keepcount ));

    return iret;
}

// end -- add by tangpengcheng on 2014/04/01
