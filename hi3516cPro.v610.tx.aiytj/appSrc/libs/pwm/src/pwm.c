...
int SU_PWM_CreateChn(uint32_t chn_num, SUPWMChnAttr *chn_attr)
{
	...
	if((chn_attr->period < 200) || (chn_attr->period > 1000000000)) {
		printf("chn_attr->period error period:%d!\n", chn_attr->period);
		return -1;
	}

	if((chn_attr->duty < 0) || (chn_attr->duty > chn_attr->period)) {
		printf("chn_attr->duty error duty:%d period:%d!\n", chn_attr->duty, chn_attr->period);
		return -1;
	}

	if((chn_attr->polarity < 0) || (chn_attr->polarity > 1)) {
		printf("chn_attr->polarity error polarity:%d!\n", chn_attr->polarity);
		return -1;
	}
...
int SU_PWM_GetChnAttr(uint32_t chn_num, SUPWMChnAttr *chn_attr)
{
	...
	if(pwm->create[chn_num] == 0) {
		printf("not SU_PWM_CreateChn chn:%d!\n", chn_num);
		return -1;
	}
...
int pwm_set_param(int index,int enable,int frequency,int duty)
{
    if (index < 0 || index > NR_MAX_PWM_NUM) {
    ...
    int ret = 0;
    SUPWMChnAttr chn_attr = {0};
    SUPWM *pwm = getpwm();
    if (NULL == pwm) {
        ret = SU_PWM_Init();
	    if(ret < 0) {
	    	printf("SU_PWM_Init chn %d error !\n", index);
	    	return -1;
	    }
        pwm = getpwm();
    }

    if (0 == pwm->create[index]) {
        chn_attr.period = frequency;
        chn_attr.duty = duty;
        chn_attr.polarity = 1;

        ret = SU_PWM_CreateChn(index, &chn_attr);
	    if(ret < 0) {
	    	printf("SU_PWM_CreateChn chn %d error !\n", index);
	    	return -1;
	    }

        ret = SU_PWM_EnableChn(index);
	    if(ret < 0) {
	    	printf("SU_PWM_EnableChn error chn %d !\n", index);
	    	return -1;
	    }
    }

    if (enable) {
        ret = SU_PWM_ModifyChnDuty(index, duty);
    } else {
        ret = SU_PWM_ModifyChnDuty(index, 0);
    }
	if(ret < 0) {
		printf("SU_PWM_ModifyChnDuty error chn %d !\n", index);
		return -1;
	}
