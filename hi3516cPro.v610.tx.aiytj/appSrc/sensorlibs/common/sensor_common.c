...
td_s32 cis_read_reg(cis_i2c *i2c, td_u32 addr)
{
    ...
    ret = write(i2c->fd, buf, i2c->addr_byte_num);
    if (ret < 0) {
        isp_err_trace("I2C_WRITE error!\n");
        return TD_FAILURE;
    }

    ret = read(i2c->fd, buf, i2c->data_byte_num);
    if (ret < 0) {
        isp_err_trace("I2C_READ error!\n");
        return TD_FAILURE;
    }

    value = buf[0];
    //printf("cis_read_reg value=0x%x\n",value);
    return value;
