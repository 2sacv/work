...
#include "sc235hai_sensor_ctrl.h"

static td_bool g_standby_cfg_init = 0;

/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/
static cis_info g_sc235hai_info[OT_ISP_MAX_PIPE_NUM] = {
    [0 ...(OT_ISP_MAX_PIPE_NUM - 1)] {
        .mutex = PTHREAD_MUTEX_INITIALIZER,
        .sns_id = SC235HAI_ID,
        .fswdr_mode = OT_ISP_FSWDR_NORMAL_MODE,
        .quick_start_en = TD_FALSE,
        .max_short_exp = SC235HAI_SEXP_MAX_DEFAULT,
        .bus_info = { .i2c_dev = -1 },
        .sns_state = TD_NULL,
        ...
        .mode_tbl = {
             /* mode 0: linear */
            {
                SC235HAI_VMAX_LINEAR,
                SC235HAI_FULL_LINES_MAX_LINEAR,
                SC235HAI_FPS_MAX_LINEAR,
                SC235HAI_FPS_MIN_LINEAR,
                SC235HAI_WIDTH_LINEAR,
                SC235HAI_HEIGHT_LINEAR,
                SC235HAI_MODE_LINEAR,
                OT_WDR_MODE_NONE,
                "SC235HAI_2M_30FPS_10BIT_LINEAR_MODE"
            },
            /* mode 1: WDR */
            {
                SC235HAI_VMAX_2TO1_LINE_WDR,
                SC235HAI_FULL_LINES_MAX_2TO1_LINE_WDR,
                SC235HAI_FPS_MAX_2TO1_LINE_WDR,
                SC235HAI_FPS_MIN_2TO1_LINE_WDR,
                SC235HAI_WIDTH_2TO1_LINE_WDR,
                SC235HAI_HEIGHT_2TO1_LINE_WDR,
                SC235HAI_MODE_2TO1_LINE_WDR,
                OT_WDR_MODE_2To1_LINE,
                "SC235HAI_2M_30FPS_10BIT_2TO1_VC_MODE"
            },
        },
        .i2c.fd = -1,
    }
 ...
 ****************************************************************************/
#define SC235HAI_DCG_RATIO 3.70f
#define SC235HAI_AGAIN_NODE_NUM               320
#define SC235HAI_AGAIN_ADDR_INDEX_NODE_NUM    320
#define SC235HAI_DGAIN_NODE_NUM               256
#define SC235HAI_DGAIN_ADDR_INDEX_NODE_NUM    256

static td_u32 again_index[SC235HAI_AGAIN_NODE_NUM] = {
...
static td_u32 dgain_index[SC235HAI_DGAIN_NODE_NUM] = {
    1024, 1040, 1056, 1072, 1088, 1104, 1120, 1136, 1152, 1168, 1184, 1200, 1216, 1232, 1248, 1264,
    1280, 1296, 1312, 1328, 1344, 1360, 1376, 1392, 1408, 1424, 1440, 1456, 1472, 1488, 1504, 1520,
    1536, 1552, 1568, 1584, 1600, 1616, 1632, 1648, 1664, 1680, 1696, 1712, 1728, 1744, 1760, 1776,
    1792, 1808, 1824, 1840, 1856, 1872, 1888, 1904, 1920, 1936, 1952, 1968, 1984, 2000, 2016, 2032,
...
static td_u32 again_table[SC235HAI_AGAIN_ADDR_INDEX_NODE_NUM] = {
...
static td_u32 dgain_table[SC235HAI_DGAIN_ADDR_INDEX_NODE_NUM] = {
...
static cis_info *cmos_get_info(ot_vi_pipe vi_pipe)
{
    ...
    return &g_sc235hai_info[vi_pipe];
}
    ...
    const ot_isp_sns_state *sns_state)
{
    ...
    td_u32 fl = SC235HAI_VMAX_LINEAR << 1;

    max_fps = cis->mode_tbl[sns_state->img_mode].max_fps;
    if (sns_state->img_mode == SC235HAI_2M_30FPS_10BIT_LINEAR_MODE) {
        ...
        ae_sns_dft->full_lines_max = SC235HAI_FULL_LINES_MAX_LINEAR << 1;
    } else if (sns_state->img_mode == SC235HAI_2M_30FPS_10BIT_2TO1_VC_MODE) {
        ae_sns_dft->int_time_accu.accu_type = OT_ISP_AE_ACCURACY_LINEAR;
        ae_sns_dft->int_time_accu.accuracy = INT_TIME_ACCURACY_WDR; /* accuracy: 4 */
        ae_sns_dft->int_time_accu.offset = 0;
        ae_sns_dft->full_lines_max = SC235HAI_FULL_LINES_MAX_2TO1_LINE_WDR << 1;
        fl = SC235HAI_VMAX_2TO1_LINE_WDR << 1;
    } else {
    }
...
static td_void cmos_get_ae_linear_default(cis_info *cis, ot_isp_ae_sensor_default *ae_sns_dft,
    const ot_isp_sns_state *sns_state)
{
    ae_sns_dft->max_again = SC235HAI_AGAIN_MAX; /* 31.75*1.54*1024 = 50069 */
    ae_sns_dft->min_again = SC235HAI_AGAIN_MIN; /* min 1024 */
    ae_sns_dft->max_again_target = ae_sns_dft->max_again;
    ae_sns_dft->min_again_target = ae_sns_dft->min_again;
    ae_sns_dft->max_dgain = SC235HAI_DGAIN_MAX; /* max 16256 */
    ae_sns_dft->min_dgain = SC235HAI_DGAIN_MIN; /* min 1024 */
    ...
    ae_sns_dft->min_int_time = 4; /* min int 8 */
...
static td_void cmos_fps_set(ot_vi_pipe vi_pipe, td_float fps, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    if ((fps > max_fps) || (fps < min_fps)) {
        isp_err_trace("ISP sensor sc235hai Not support Fps: %f\n", fps);
        return;
    }
...
static td_void cmos_slow_framerate_set(ot_vi_pipe vi_pipe, td_u32 full_lines, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    vmax = ((vmax + 1) >> 1) << 1;
    vmax = clip3(vmax, SC235HAI_VMAX_LINEAR, lines_max);
    ...
    cis_info *cis, ot_isp_sns_state *sns_state, td_u32 int_time)
{
    ...
    if (is_first[vi_pipe]) { /* short exposure */
        ...
        max_short_exp = MAX2((short_int_time[vi_pipe] + FL_OFFSET_WDR_SHORT) >> 1, SC235HAI_SEXP_MAX_DEFAULT);
        max_next_short_exp = (SC235HAI_EXP_Y + 20) * 2 + (td_s32)cis->max_short_exp - /* 20 2 */
...
static td_void cmos_again_calc_table(ot_vi_pipe vi_pipe, td_u32 *again_lin, td_u32 *again_db)
{
    ...
    if (*again_lin >= (td_u32)(again_index[SC235HAI_AGAIN_ADDR_INDEX_NODE_NUM - 1] * SC235HAI_DCG_RATIO)) {
        *again_lin = (td_u32)(again_index[SC235HAI_AGAIN_ADDR_INDEX_NODE_NUM - 1] * SC235HAI_DCG_RATIO);
        if (SC235HAI_DCG_RATIO > 1) {
            *again_db = again_table[SC235HAI_AGAIN_NODE_NUM - 1] | 0x8000;
        } else {
            *again_db = again_table[SC235HAI_AGAIN_NODE_NUM - 1];
        }
        return;
    }
    if (*again_lin > (1024 * SC235HAI_DCG_RATIO)) {  /* 1024 */
        again_lin_tmp = (td_u32)(*again_lin / SC235HAI_DCG_RATIO);
        for (i = 1; i < SC235HAI_AGAIN_NODE_NUM; i++) {
            if (again_lin_tmp < again_index[i]) {
                *again_lin = again_index[i - 1] * SC235HAI_DCG_RATIO;
...
static td_void cmos_dgain_calc_table(ot_vi_pipe vi_pipe, td_u32 *dgain_lin, td_u32 *dgain_db)
{
    ...
    if (*dgain_lin >= dgain_index[SC235HAI_DGAIN_ADDR_INDEX_NODE_NUM - 1]) {
        *dgain_lin = dgain_index[SC235HAI_DGAIN_ADDR_INDEX_NODE_NUM - 1];
        *dgain_db = dgain_table[SC235HAI_DGAIN_NODE_NUM - 1];
        return;
    }

    for (i = 1; i < SC235HAI_DGAIN_NODE_NUM; i++) {
...
static td_void cmos_isp_get_dng_default(const ot_isp_sns_state *sns_state, ot_isp_cmos_default *isp_def)
{
    ...
    switch (sns_state->img_mode) {
        case SC235HAI_2M_30FPS_10BIT_LINEAR_MODE:
        ...
        case SC235HAI_2M_30FPS_10BIT_2TO1_VC_MODE:
...
static td_s32 cmos_isp_get_default(ot_vi_pipe vi_pipe, ot_isp_cmos_default *isp_def)
{
    ...
    isp_def->sns_mode.sns_id = SC235HAI_ID;
...
static td_s32 cmos_isp_set_wdr_mode(ot_vi_pipe vi_pipe, td_u8 mode)
{
    ...
    switch (mode & 0x3F) {
        case OT_WDR_MODE_NONE:
            ...
            printf("linear 2M30fps mode\n");
            break;

        case OT_WDR_MODE_2To1_LINE:
            sns_state->wdr_mode = OT_WDR_MODE_2To1_LINE;
            printf("2to1 line WDR 2M30fps mode!\n");
            break;
...
static td_void cmos_comm_sns_reg_info_init(cis_info *cis, ot_isp_sns_state *sns_state)
{
    ...
    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        ...
        sns_state->regs_info[0].i2c_data[i].dev_addr = SC235HAI_I2C_ADDR;
        sns_state->regs_info[0].i2c_data[i].addr_byte_num = SC235HAI_ADDR_BYTE;
        sns_state->regs_info[0].i2c_data[i].data_byte_num = SC235HAI_DATA_BYTE;
    }

    /* Linear Mode Regs */
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].reg_addr = SC235HAI_EXPO_L_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_M_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_M_IDX].reg_addr = SC235HAI_EXPO_M_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].reg_addr = SC235HAI_EXPO_H_ADDR;

    sns_state->regs_info[0].i2c_data[AGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[AGAIN_L_IDX].reg_addr = SC235HAI_AGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[AGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[AGAIN_H_IDX].reg_addr = SC235HAI_AGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[DGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[DGAIN_H_IDX].reg_addr = SC235HAI_DGAIN_H_ADDR;
    sns_state->regs_info[0].i2c_data[DGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[DGAIN_L_IDX].reg_addr = SC235HAI_DGAIN_L_ADDR;

    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].reg_addr = SC235HAI_VMAX_L_ADDR;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].reg_addr = SC235HAI_VMAX_H_ADDR;
    return;
}
...
static td_void cmos_2to1_line_wdr_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_L_IDX].reg_addr = SC235HAI_SHORT_EXPO_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_M_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_M_IDX].reg_addr = SC235HAI_SHORT_EXPO_M_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].reg_addr = SC235HAI_SHORT_AGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].reg_addr = SC235HAI_SHORT_AGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].reg_addr = SC235HAI_SHORT_DGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].reg_addr = SC235HAI_SHORT_DGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_L_ADDR_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_L_ADDR_IDX].reg_addr = SC235HAI_SHORT_EXP_MAX_L_ADDR;
    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_H_ADDR_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_H_ADDR_IDX].reg_addr = SC235HAI_SHORT_EXP_MAX_H_ADDR;
    return;
}
...
static td_s32 cmos_isp_get_standby_cfg(ot_vi_pipe vi_pipe, ot_isp_sns_regs_info *standby_cfg)
{
    ...
    ret = sc235_get_standby_cfg(standby_cfg);
    if (ret == TD_SUCCESS) {
        g_standby_cfg_init = TD_TRUE;
    }
    ...
    ot_isp_sns_state *sns_state)
{
    ...
    switch (sns_image_mode) {
        case SC235HAI_2M_30FPS_10BIT_LINEAR_MODE:
            sns_state->fl_std = SC235HAI_VMAX_LINEAR * 2; /* 2 */
            break;
        case SC235HAI_2M_30FPS_10BIT_2TO1_VC_MODE:
            sns_state->fl_std = SC235HAI_VMAX_2TO1_LINE_WDR * 2; /* 2 */
            break;
        default:
            sns_state->fl_std = SC235HAI_VMAX_LINEAR * 2; /* 2 */
            break;
...
static td_s32 cmos_isp_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sns_image_mode *sns_image_mode)
{
    ...
    for (i = 0; i < SC235HAI_MODE_MAX; i++) {
        if (sns_image_mode->fps <= cis->mode_tbl[i].max_fps &&
            ...
            image_mode = (sc235hai_res_mode)i;
            break;
        }
    }

    if (i >= SC235HAI_MODE_MAX) {
...
static void cmos_isp_init(ot_vi_pipe vi_pipe)
{
    ...
    cis->i2c.addr = SC235HAI_I2C_ADDR;
    cis->i2c.addr_byte_num = SC235HAI_ADDR_BYTE;
    cis->i2c.data_byte_num = SC235HAI_DATA_BYTE;
    ...
    if (OT_WDR_MODE_2To1_LINE == wdr_mode) {
        if (SC235HAI_2M_30FPS_10BIT_2TO1_VC_MODE == img_mode) {
            sc235hai_vc_wdr_2t1_2m30_10bit_init(cis);
        } else {
        }
    } else {
        sc235hai_linear_2m30_10bit_init(cis);
    }
...
static td_void cmos_isp_global_init(ot_vi_pipe vi_pipe)
{
    ...
    sns_state->sync_init = TD_FALSE;
    sns_state->img_mode  = SC235HAI_2M_30FPS_10BIT_LINEAR_MODE;
    sns_state->wdr_mode  = OT_WDR_MODE_NONE;
    sns_state->fl_std    = SC235HAI_VMAX_LINEAR << 1;
    sns_state->fl[0]     = SC235HAI_VMAX_LINEAR << 1;
    sns_state->fl[1]     = SC235HAI_VMAX_LINEAR << 1;
...
static td_void cmos_isp_set_pixel_detect(ot_vi_pipe vi_pipe, td_bool enable)
{
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_2To1_LINE) {
        ...
        if (sns_state->img_mode == SC235HAI_2M_30FPS_10BIT_LINEAR_MODE) {
            full_lines_5fps = (SC235HAI_VMAX_LINEAR * STANDARD_FPS) / 5; /* 30fps, 5fps */
        } else {
            return;
        }
    }
    ...
    if (enable) { /* setup for ISP pixel calibration mode */
        cis_write_reg(i2c, SC235HAI_AGAIN_L_ADDR, 0x40);
        cis_write_reg(i2c, SC235HAI_AGAIN_H_ADDR, 0x03);

        cis_write_reg(i2c, SC235HAI_DGAIN_H_ADDR, 0x00);
        cis_write_reg(i2c, SC235HAI_DGAIN_L_ADDR, 0x80);

        cis_write_reg(i2c, SC235HAI_VMAX_L_ADDR, low_8bits(full_lines_5fps));
        cis_write_reg(i2c, SC235HAI_VMAX_H_ADDR, high_8bits(full_lines_5fps));

        cis_write_reg(i2c, SC235HAI_EXPO_L_ADDR, lower_4bits(full_lines_5fps - 10)); /* sub 10 */
        cis_write_reg(i2c, SC235HAI_EXPO_M_ADDR, higher_8bits(full_lines_5fps - 10)); /* sub 10 */
        cis_write_reg(i2c, SC235HAI_EXPO_H_ADDR, higher_4bits(full_lines_5fps - 10)); /* sub 10 */
    } else { /* setup for ISP 'normal mode' */
        sns_state->fl_std = (sns_state->fl_std > SC235HAI_FULL_LINES_MAX_LINEAR) ?
            SC235HAI_FULL_LINES_MAX_LINEAR : sns_state->fl_std;
        cis_write_reg(i2c, SC235HAI_VMAX_L_ADDR, low_8bits(sns_state->fl_std));
        cis_write_reg(i2c, SC235HAI_VMAX_H_ADDR, high_8bits(sns_state->fl_std));
        sns_state->sync_init = TD_FALSE;
    }
    return;
}
...
static void cmos_restart(ot_vi_pipe vi_pipe)
{
    ...
    ret += cis_write_reg(&cis->i2c, 0x302c, 0x00);
    if (ret != TD_SUCCESS) {
        isp_err_trace("write register failed!\n");
    }

    ret += cis_write_reg(&cis->i2c, 0x3018, 0x3a);
    if (ret != TD_SUCCESS) {
        isp_err_trace("write register failed!\n");
    }

    ret += cis_write_reg(&cis->i2c, 0x3019, 0x00);
    if (ret != TD_SUCCESS) {
        isp_err_trace("write register failed!\n");
    }
...
static void cmos_mirror_flip(ot_vi_pipe vi_pipe, ot_isp_sns_mirrorflip_type sns_mirror_flip)
{
    td_s32 value = 0x99;
    cis_info *cis = TD_NULL;
    cis_i2c *i2c = TD_NULL;
    ...
    default:
    case ISP_SNS_NORMAL:
        cis_write_reg(i2c, 0x3221, 0x00 | value);
        break;

    case ISP_SNS_MIRROR:
        cis_write_reg(i2c, 0x3221, 0x06 | value);    //bit[2:1]
        break;

    case ISP_SNS_FLIP:
        cis_write_reg(i2c, 0x3221, 0x60 | value);    //bit[6:5]
        break;

    case ISP_SNS_MIRROR_FLIP:
        cis_write_reg(i2c, 0x3221, 0x66 | value);
        break;
    }
    return;
}
...
static td_s32 cmos_read_register(ot_vi_pipe vi_pipe, td_u32 addr)
{
    ...
    value = cis_read_reg(&cis->i2c, addr);
...
ot_isp_sns_obj g_sns_sc235hai_obj = {
...
ot_isp_sns_obj *sc235hai_get_obj(td_void)
{
