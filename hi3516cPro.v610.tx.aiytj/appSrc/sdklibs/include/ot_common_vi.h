...
typedef enum {
    OT_VI_NR_EFFECT_MODE_NORM = 0,
    OT_VI_NR_EFFECT_MODE_ADVANCE,
    OT_VI_NR_EFFECT_MODE_SWITCH,

    OT_VI_NR_EFFECT_MODE_BUTT
} ot_nr_effect_mode;
...
typedef struct {
    ...
    ot_nr_effect_mode nr_effect_mode; /* RW; 3dnr effect mode */
    td_bool aibnr_debug_en;       /* RW; support pipe send raw in aibnr mode */
} ot_vi_pipe_param;
...
typedef struct {
    ...
    td_u32 buf_line;    /* RW; Range: [128, pipe_height * 2] */
} ot_vi_pipe_buf_wrap_attr;
...
typedef struct {
    ...
    td_u32              depth;           /* RW; range [0,8];depth */
    ot_frame_rate_ctrl  frame_rate_ctrl; /* RW; frame rate ctrl */
    td_bool             buf_share_en;    /* RW; ext chn share phys chn buffer */
} ot_vi_ext_chn_attr;
...
typedef struct {
    td_bool             distribute_en;                           /* RW; distribute enable */
    ot_vi_pipe          pipe_id[OT_VI_MAX_DISTRIBUTE_PIPE_NUM];  /* RW; Array of pipe ID */
} ot_vi_distribute_grp_attr;

typedef enum {
    OT_VI_SWITCH_SENSOR_0 = 0,
    OT_VI_SWITCH_SENSOR_1,
    OT_VI_SWITCH_SENSOR_BUTT
} ot_vi_switch_sensor_select;

typedef struct {
    td_bool                      switch_sensor_en;       /* RW; switch sensor mode enable */
    ot_vi_switch_sensor_select   switch_sensor_select;   /* RW; switch sensor mode select sensor */
} ot_vi_switch_sensor_attr;
