...
#define AIBNR_BUFFER_SHARE_EXPAN_WIDTH 128
#define AIBNR_BUFFER_SHARE_ADDR_OFFSET (AIBNR_BUFFER_SHARE_EXPAN_WIDTH * 12 / 8) /* 12/8 aiisp only support raw12 */
...
__inline static td_u32 ot_aibnr_get_vb_size(td_u32 width, td_u32 height, ot_compress_mode compress_mode)
{
    ...
    buf_attr.width         = OT_ALIGN_UP((width + AIBNR_BUFFER_SHARE_EXPAN_WIDTH), 4); /* align up to 4 */
...
__inline static td_u32 ot_venc_get_h265_tmv_size(td_u32 width, td_u32 pic_height)
{
...
__inline static td_u32 ot_venc_get_h264_pic_buf_size(const ot_venc_buf_attr *attr)
{
    ...
    pme_share_en = (attr->share_buf_en && (attr->svc_version != OT_VENC_SVC_V2)) ? TD_TRUE : TD_FALSE;
