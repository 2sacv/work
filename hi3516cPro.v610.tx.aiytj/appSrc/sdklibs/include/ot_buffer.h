...
__inline static td_u32 ot_comm_get_vpss_venc_wrap_buf_size(const ot_pic_buf_attr *buf_attr, td_u32 buf_line)
{
    ot_vb_calc_cfg calc_cfg;
    ot_pic_buf_attr tmp_buf_attr;

    if (buf_attr == TD_NULL) {
        return 0;
    }

    (td_void)memcpy_s(&tmp_buf_attr, sizeof(ot_pic_buf_attr), buf_attr, sizeof(ot_pic_buf_attr));
    tmp_buf_attr.height = buf_line;
    ot_common_get_pic_buf_cfg(&tmp_buf_attr, &calc_cfg);

    if (buf_line != 0 && buf_line < buf_attr->height) {
        if (buf_attr->compress_mode == OT_COMPRESS_MODE_SEG_COMPACT) {
            ot_vb_calc_cfg tmp_calc_cfg;

            ot_common_get_pic_buf_cfg(buf_attr, &tmp_calc_cfg); /* only use head size */
            calc_cfg.vb_size = calc_cfg.main_size + tmp_calc_cfg.head_size;
        }
    }
...
__inline static td_u32 ot_venc_get_pic_buf_size(ot_payload_type type, const ot_venc_buf_attr *attr)
{
    ...
    switch (type) {
        case OT_PT_H264:
            return ot_venc_get_h264_pic_buf_size(attr);
        case OT_PT_H265:
        case OT_PT_SVAC3:
            return ot_venc_get_h265_pic_buf_size(attr);
        default:
            return 0;
    }
}
...
__inline static td_u32 ot_venc_get_pic_info_buf_size(ot_payload_type type, const ot_venc_buf_attr *attr)
{
    ...
    pme_share_en = ((attr->share_buf_en == TD_TRUE) && (attr->svc_version != OT_VENC_SVC_V2)) ? TD_TRUE : TD_FALSE;
...
__inline static td_u32 ot_venc_get_qpmap_stride(td_u32 width)
{
    ...
    return OT_ALIGN_UP(width, 128) / 16;
}
...
__inline static td_u32 ot_venc_get_skip_weight_stride(ot_payload_type type, td_u32 width)
{
    switch (type) {
        case OT_PT_H264:
            return (OT_ALIGN_UP(width, 256) / 32);
        case OT_PT_H265:
        case OT_PT_SVAC3:
            return (OT_ALIGN_UP(width, 512) / 64);
        default:
            return  0;
    }
}
...
__inline static td_u32 ot_venc_get_skip_weight_size(ot_payload_type type, td_u32 width, td_u32 height)
{
    ...
    if (type == OT_PT_H265 || type == OT_PT_SVAC3) {
...
__inline static td_u32 ot_venc_get_roimap_stride(ot_payload_type type, td_u32 width)
{
    ...
    if (type == OT_PT_JPEG || type == OT_PT_MJPEG) {
        stride = OT_ALIGN_UP(width, 512) / 32; // 512 32: hardware align requirement
    } else {
        stride = 0;
    }
...
__inline static td_u32 ot_venc_get_mosaic_map_stride(td_u32 width, td_u32 blk_size)
{
    ...
    return OT_ALIGN_UP(blk_width, 64) / 8; // 64, 8: hardware align requirement
}
...
__inline static td_u32 ot_venc_get_svc_mask_size(td_u32 width, td_u32 height)
{
    td_u32 stride;

    if (width > OT_VENC_H265_MAX_WIDTH || height > OT_VENC_H265_MAX_HEIGHT) {
        return 0;
    }

    stride = OT_ALIGN_UP(width, 2); /* 2: align requirement */
    return stride * height / 2; /* 2 pixels 1 byte */
}
...
__inline static td_u32 ot_vpss_get_mcf_color_buf_size(td_u32 width, td_u32 height)
{
    ...
    align_height = OT_ALIGN_UP(height, 8); /* 8: align requirement */
    size = stride * align_height * 29 / 16;
...
__inline static td_u32 ot_vpss_get_mcf_mono_buf_size(td_u32 width, td_u32 height)
{
    ...
    size = stride * align_height * 21 / 16;
