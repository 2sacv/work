...
uint8_t g_local_name[ NAME_MAX_LENGTH] = {0};
...
static uint8_t ble_set_adv_name(uint8_t *set_adv_data_position, uint8_t max_len)
{
    ...
    conf_get_wificfg(&wifi_cfg);
    for (i=0; i<17;i++) {
        if (i ==9 || i ==10 || i == 12 || i==13 || i == 15 || i == 16) {
            mac_data[j++] = wifi_cfg.mac[i];
        }
    }
    DBG("mac_data = %s, date.mac=%s\n", mac_data, wifi_cfg.mac);
    int appid = 0;
    SysInfoS sysinfo = {0};
    conf_get_sysinfocfg(&sysinfo);
    appid = atoi10(sysinfo.custom_appid)%100;
    if (appid == 0) {   // 中性版本兼容当前已提交上线的大卫看家app版本
        snprintf((char *)g_local_name, sizeof(g_local_name), "AIBLE-%s", mac_data);
    } else {
        snprintf((char *)g_local_name, sizeof(g_local_name), "AIBLE%d-%s", appid, mac_data);
    }
