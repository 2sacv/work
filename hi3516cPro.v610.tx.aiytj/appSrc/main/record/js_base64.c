...
static unsigned char * base64_malloc(int iSize)
{
    return (unsigned char *)malloc(iSize);
}
...
int js_b64_decode(unsigned char *data,int dataLen, unsigned char **decData,int *decDataLen)
{
    ...
    *decData = newstr = base64_malloc(*decDataLen+4);
    if(newstr == NULL)
        return 0;
...
int js_b64_encode(unsigned char *data,int dataLen,unsigned char **encData,int *encDataLen)
{
    ...
    *encData = output = base64_malloc(dataLen*4/3+4);
    if(NULL == output)
        return 0;
    ...
    *encDataLen = strlen((char*)*encData);
    ...
    return 1; /* return the length of the new data */
}
