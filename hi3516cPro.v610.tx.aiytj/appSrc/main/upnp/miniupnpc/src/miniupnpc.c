/* $Id: miniupnpc.c,v 1.73 2010/01/06 10:03:53 nanard Exp $ */
/* Project : miniupnp
            ...
            case '\x0d':
                if(b!=0) {
                    ...
                    if(0==strncasecmp(reply+a, "location", 8)) {
                        ...
                        *usn = reply+b;
                        *usnsize = i-b;
                    } else if(0==strncasecmp(reply+a, "server", 6)) {
                        *server = reply+b;
                        *serversize = i-b;
                    }
                    b = 0;
...
LIBSPEC struct UPNPDev * upnpDiscover(int delay, const char * multicastif,
    ...
    if(GetBestRoute(inet_addr("223.255.255.255"), 0, &ip_forward) == NO_ERROR) {
       ...
       if(pIPAddrTable) {
           ...
           printf("\tIP Address[%d]:     \t%s\n", i, j_inet_ntoa(IPAddr) );
           IPAddr.S_un.S_addr = (u_long) pIPAddrTable->table[i].dwMask;
           printf("\tSubnet Mask[%d]:    \t%s\n", i, j_inet_ntoa(IPAddr) );
           IPAddr.S_un.S_addr = (u_long) pIPAddrTable->table[i].dwBCastAddr;
           printf("\tBroadCast[%d]:      \t%s (%ld)\n", i, j_inet_ntoa(IPAddr), pIPAddrTable->table[i].dwBCastAddr);
    ...
    for(n = 0;;) {
        if(n == 0) {
            ...
            if (n < 0) {
                ...
                if (SUCCESS != NetSendToBroadCast(sudp, bufr, len, &sockudp_w)) {
                    closesocket(sudp);
                    return devlist;
                }
            }
        ...
        if (n < 0) {
            ...
            if(st&&descURL) {
                ...
                tmp = (struct UPNPDev *)malloc(sizeof(struct UPNPDev)+urlsize+stsize+usnsize+serversize);
                tmp->pNext = devlist;
                offset = 0;
                tmp->descURL = tmp->buffer + offset;
                memcpy(tmp->buffer + offset, descURL, urlsize);
                offset += urlsize;
                tmp->buffer[offset++] = '\0';

                tmp->st = tmp->buffer + offset;
                memcpy(tmp->buffer + offset, st, stsize);
                offset += stsize;
                tmp->buffer[offset++] = '\0';

                if (usn) {
                    tmp->usn = tmp->buffer + offset;
                    memcpy(tmp->buffer + offset, usn, usnsize);
                    offset += usnsize;
                    tmp->buffer[offset++] = '\0';
                } else {
                    tmp->usn = NULL;
                ...
                if (server) {
                    tmp->server = tmp->buffer + offset;
                    memcpy(tmp->buffer + offset, server, serversize);
                    offset += serversize;
                    tmp->buffer[offset++] = '\0';
                } else {
                    tmp->server = NULL;
                ...
                if(descXML) {
                    ...
                    if (bConnected) {
                        ...
                        if (7 > strlen(wanaddr) // "1.1.1.1" strlen == 7
                            || bSpace
                            || strstr(wanaddr, "0.0.0.0")) {
                            // avoid the route at lan, not have wan ip
                            FreeUPNPUrls(urls);
                            memset(data, 0, sizeof(struct IGDdatas));
                            continue;
                        }
