...
#define DDNS_DYN_SERVER_DOMAIN  "members.dyndns.org"

int ddns_dyn_update(ddns_opt_t *opt, int timeout, char status[/*32*/])
{
	assert(status);
	char ipaddr[16];
	if(!net_domain_to_ipaddr(DDNS_DYN_SERVER_DOMAIN, IPV4, TCP, ipaddr, sizeof(ipaddr)))
		return -1;
	...
	len = sprintf(ddnsbuf, "GET /nic/update?hostname=%s HTTP/1.1\r\n"
			"Host: %s\r\n"
			"Authorization: Basic %s\r\n"
			"User-Agent: jabsco_nxp/1.0\r\n\r\n", opt->sdomain, DDNS_DYN_SERVER_DOMAIN, base64_auth);
	...
	if(net_sendn(&sockobj, ddnsbuf, len) < 0){
