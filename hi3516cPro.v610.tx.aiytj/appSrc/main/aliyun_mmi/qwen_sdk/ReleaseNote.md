...
2. 支持workspace license模式

3. 增加incremental_response、response_text配置选项，transmit_rate_limit增加自适应默认配置。

4. 增加海外接入节点c_mmi_get_wss_host_global。

5. 优化存储结构，统一license与非license存储结构。

6. 修复 token 未过期时返回错误问题。

7. 增加agent子模块，支持图片问答、生图等功能。

8. 优化requestToRespond业务逻辑，支持任意时刻调用打断当前交互。

## 芯片平台相关注意事项

ec718pm和asr1606芯片，在获取llm返回文本的时候，原默认取text字段；

更新SDK后需要通过mmi_user_config_t.response_text进行配置。
...
`c_mm_cmd_register` 入参变更，由

```cpp
int32_t c_mm_cmd_register(char *domain_name, char *cmd, c_multi_modal_func func)
