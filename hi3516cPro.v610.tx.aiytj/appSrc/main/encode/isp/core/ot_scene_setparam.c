...
static td_char g_dir_name[DIR_NAME_BUFF];

static ot_scene_fps g_sceneauto_fps = {0};
...
static fpn_correction_cfg g_correction_cfg = {
...
td_s32 ot_scene_set_static_ae(ot_vi_pipe vi_pipe, td_u8 index)
{
    ...
    for (td_u32 i = 0; i < ae_route_ex.total_num && i < OT_ISP_AE_ROUTE_EX_MAX_NODES; i++) {
...
td_s32 ot_scene_set_static_ccm(ot_vi_pipe vi_pipe, td_u8 index)
{
    ...
    ot_isp_color_tone_attr  color_tone_attr;
    ...
    for (i = 0; i < get_pipe_params()[index].static_ccm.total_num && i < OT_ISP_CCM_MATRIX_NUM; i++) {
    ...
    ret = ss_mpi_isp_get_color_tone_attr(vi_pipe, &color_tone_attr);
    check_scene_ret(ret);
    ...
    ret = ss_mpi_isp_set_color_tone_attr(vi_pipe, &color_tone_attr);
    check_scene_ret(ret);
    return TD_SUCCESS;
}
...
{
    ot_scenecomm_expr_true_return(index >= OT_SCENE_PIPETYPE_NUM, TD_FAILURE);
    if (get_pipe_params()[index].module_state.static_color_sector != TD_TRUE) {
        return TD_SUCCESS;
    }

    td_u32 i, j;
    td_s32 ret;
    ot_isp_color_sector_attr color_sector_attr;

    ret = ss_mpi_isp_get_color_sector_attr(vi_pipe, &color_sector_attr);
    check_scene_ret(ret);
    color_sector_attr.enable = get_pipe_params()[index].static_color_sector.enable;

    for (i = 0; i < OT_ISP_CCM_MATRIX_NUM; i++) {
    ...
    ret = ss_mpi_isp_set_color_sector_attr(vi_pipe, &color_sector_attr);
    check_scene_ret(ret);
    return TD_SUCCESS;
}
...
static td_s32 ot_scene_set_static_nr_snr(ot_vi_pipe vi_pipe, td_u8 index, ot_isp_nr_attr *nr_attr)
{
    ...
    for (i = 0; i < OT_ISP_AUTO_ISO_NUM; i++) {
        ...
        nr_attr->snr_cfg.snr_attr.snr_auto.coring_wgt[i] = get_pipe_params()[index].static_nr.snr_cfg.coring_wgt[i];
...
static td_s32 ot_scene_set_static_nr_md(ot_vi_pipe vi_pipe, td_u8 index, ot_isp_nr_attr *nr_attr)
{
    td_s32 i;
    ...
    for (i = 0; i < OT_ISP_AUTO_ISO_NUM; i++) {
        ...
        nr_attr->md_cfg.md_auto.tfs[i] = get_pipe_params()[index].static_nr.md_cfg.tfs[i];
    }
    return TD_SUCCESS;
}
...
td_s32 ot_scene_set_static_nr(ot_vi_pipe vi_pipe, td_u8 index)
{
    ...
    nr_attr.ref_mode = get_pipe_params()[index].static_nr.bnr_ref_mode;
    nr_attr.load_ref_en = get_pipe_params()[index].static_nr.load_ref_en;
...
td_s32 ot_scene_set_static_sharpen(ot_vi_pipe vi_pipe, td_u8 index)
{
    ...
    ot_isp_sharpen_auto_attr *auto_attr = &sharpen_attr.auto_attr;
    ...
    (td_void)memcpy_s(auto_attr->edge_rly_attr.edge_gain_by_rly, OT_ISP_SHARPEN_RLYWGT_NUM * OT_ISP_AUTO_ISO_NUM,
        static_sharpen->sharpen_auto_cfg.edge_gain_by_rly,  OT_ISP_SHARPEN_RLYWGT_NUM * OT_ISP_AUTO_ISO_NUM);

    (td_void)memcpy_s(auto_attr->edge_rly_attr.edge_rly_by_mot, OT_ISP_SHARPEN_STDGAIN_NUM * OT_ISP_AUTO_ISO_NUM,
        static_sharpen->sharpen_auto_cfg.edge_rly_by_mot,  OT_ISP_SHARPEN_STDGAIN_NUM * OT_ISP_AUTO_ISO_NUM);

    (td_void)memcpy_s(auto_attr->edge_rly_attr.edge_rly_by_luma, OT_ISP_SHARPEN_STDGAIN_NUM * OT_ISP_AUTO_ISO_NUM,
        static_sharpen->sharpen_auto_cfg.edge_rly_by_luma,  OT_ISP_SHARPEN_STDGAIN_NUM * OT_ISP_AUTO_ISO_NUM);

    (td_void)memcpy_s(auto_attr->gain_by_mot_attr.mf_gain_by_mot, OT_ISP_SHARPEN_MOT_NUM * OT_ISP_AUTO_ISO_NUM,
        static_sharpen->sharpen_auto_cfg.mf_gain_by_mot,  OT_ISP_SHARPEN_MOT_NUM * OT_ISP_AUTO_ISO_NUM);

    (td_void)memcpy_s(auto_attr->gain_by_mot_attr.hf_gain_by_mot, OT_ISP_SHARPEN_MOT_NUM * OT_ISP_AUTO_ISO_NUM,
        static_sharpen->sharpen_auto_cfg.hf_gain_by_mot,  OT_ISP_SHARPEN_MOT_NUM * OT_ISP_AUTO_ISO_NUM);

    (td_void)memcpy_s(auto_attr->gain_by_mot_attr.lmf_gain_by_mot, OT_ISP_SHARPEN_MOT_NUM * OT_ISP_AUTO_ISO_NUM,
        static_sharpen->sharpen_auto_cfg.lmf_gain_by_mot,  OT_ISP_SHARPEN_MOT_NUM * OT_ISP_AUTO_ISO_NUM);
...
td_s32 ot_scene_set_dynamic_ldci(ot_vi_pipe vi_pipe, td_u64 exposure, td_u64 last_exposure, td_u8 index)
{
    ...
    if (exposure != last_exposure) {
        ...
        exp_level = scene_get_level_ltoh(exposure, get_pipe_params()[index].dynamic_ldci.enable_cnt,
            get_pipe_params()[index].dynamic_ldci.enable_exp_thresh_ltoh, OT_SCENE_LDCI_EXPOSURE_MAX_COUNT);
...
static td_void ot_scene_set_fps_param_board(td_u32 exposure, td_u32 *frame_rate, td_u32 *ae_time, td_u32 *gop,
    td_u8 index)
{
    td_u32 exp_level;
    exp_level = scene_get_level_ltoh(exposure, get_pipe_params()[index].dynamic_fps.fps_exposure_cnt,
        get_pipe_params()[index].dynamic_fps.exp_ltoh_thresh, OT_SCENE_FPS_ISO_MAX_COUNT);
...
static td_void ot_scene_set_pub_ae_venc(ot_vi_pipe vi_pipe, td_u32 frame_rate, td_u32 ae_time, td_u32 gop)
{
    td_s32 ret;
    ...
    int dst_frame_rate = 0;
    int dst_gop = 0;
    VideoEncS video_param = {0};
    conf_get_realvideocfg(&video_param);

    for (int i = 0; i < CH_FS_H26X_END; i++){
        ret = ss_mpi_venc_get_chn_attr(i, &venc_attr);
        check_scene_ret(ret);

        if(video_param.enc[i].fps >= frame_rate) {
            dst_frame_rate = frame_rate;
            dst_gop = gop;
        } else {
            dst_frame_rate = video_param.enc[i].fps;
            dst_gop = video_param.enc[i].gop;
        }

        if (venc_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H265_CBR) {
            venc_attr.rc_attr.h265_cbr.dst_frame_rate = dst_frame_rate;
            venc_attr.rc_attr.h265_cbr.src_frame_rate = frame_rate;
            venc_attr.rc_attr.h265_cbr.gop = dst_gop;
        } else if (venc_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H265_VBR) {
            venc_attr.rc_attr.h265_vbr.dst_frame_rate = dst_frame_rate;
            venc_attr.rc_attr.h265_vbr.src_frame_rate = frame_rate;
            venc_attr.rc_attr.h265_vbr.gop = dst_gop;
        } else if (venc_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H265_AVBR) {
            venc_attr.rc_attr.h265_avbr.dst_frame_rate = dst_frame_rate;
            venc_attr.rc_attr.h265_avbr.src_frame_rate = frame_rate;
            venc_attr.rc_attr.h265_avbr.gop = dst_gop;
        } else if (venc_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H265_CVBR) {
            venc_attr.rc_attr.h265_cvbr.dst_frame_rate = dst_frame_rate;
            venc_attr.rc_attr.h265_cvbr.src_frame_rate = frame_rate;
            venc_attr.rc_attr.h265_cvbr.gop = dst_gop;
        } else if (venc_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H264_CBR) {
            venc_attr.rc_attr.h264_cbr.dst_frame_rate = dst_frame_rate;
            venc_attr.rc_attr.h264_cbr.src_frame_rate = frame_rate;
            venc_attr.rc_attr.h264_cbr.gop = dst_gop;
        }  else if (venc_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H264_VBR) {
            venc_attr.rc_attr.h264_vbr.dst_frame_rate = dst_frame_rate;
            venc_attr.rc_attr.h264_vbr.src_frame_rate = frame_rate;
            venc_attr.rc_attr.h264_vbr.gop = dst_gop;
        }  else if (venc_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H264_AVBR) {
            venc_attr.rc_attr.h264_avbr.dst_frame_rate = dst_frame_rate;
            venc_attr.rc_attr.h264_avbr.src_frame_rate = frame_rate;
            venc_attr.rc_attr.h264_avbr.gop = dst_gop;
        } else if (venc_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H264_CVBR) {
            venc_attr.rc_attr.h264_cvbr.dst_frame_rate = dst_frame_rate;
            venc_attr.rc_attr.h264_cvbr.src_frame_rate = frame_rate;
            venc_attr.rc_attr.h264_cvbr.gop = dst_gop;
        } else {
            printf("Warning:SceneAuto now only support H265/H264 CBR/AVBR/CVBR FPS settings.\n");
        }
        ret = ss_mpi_venc_set_chn_attr(i, &venc_attr);
        check_scene_ret(ret);

        ret = encode_venc_chn_frame_strategy(i);
        check_scene_ret(ret);
    }
}
...
td_s32 ot_scene_set_dynamic_ae(ot_vi_pipe vi_pipe, td_u64 exposure, td_u64 last_exposure, td_u8 index)
{
    ...
    if ((exposure != last_exposure) || (last_ratio[vi_pipe] != actual_ratio) || force_change) {
        check_scene_return_if_pause();
        ret = ss_mpi_isp_get_exposure_attr(vi_pipe, &exposure_attr);
        check_scene_ret(ret);
        ...
        exp_level = scene_get_level_ltoh(exposure, get_pipe_params()[index].dynamic_ae.ae_exposure_cnt,
            get_pipe_params()[index].dynamic_ae.exp_ltoh_thresh, OT_SCENE_AE_EXPOSURE_MAX_COUNT);
...
td_s32 ot_scene_set_dynamic_drc(ot_vi_pipe vi_pipe, td_u8 index, td_u32 wdr_ratio, td_u32 iso)
{
    ...
    if (pub_attr.wdr_mode == OT_WDR_MODE_2To1_FRAME) {
        ratio_level = scene_get_level_ltoh_u32(inner_state_info.wdr_exp_ratio_actual[0], ratio_count,
            ratio_level_thresh, OT_SCENE_DRC_RATIO_MAX_COUNT);
    } else {
        ratio_level = scene_get_level_ltoh_u32(wdr_ratio, ratio_count, ratio_level_thresh,
            OT_SCENE_DRC_RATIO_MAX_COUNT);
    }
    iso_level = scene_get_level_ltoh_u32(iso, iso_count, iso_level_thresh, OT_SCENE_DRC_ISO_MAX_COUNT);
...
static td_s32 ot_scene_set_dynamic_aibnr_part(ot_vi_pipe vi_pipe, td_u8 index)
{
    td_s32 ret;
    td_u32 i;
    td_bool is_wdr_mode;
    ...
    ot_aibnr_model_attr aibnr_model_attr;
    ret = ss_mpi_aibnr_get_model_attr(is_wdr_mode, &aibnr_model_attr);
    check_scene_ret(ret);
    ...
    for (i = 0; i < OT_AIISP_AUTO_ISO_NUM; i++) {
        aibnr_model_attr.auto_id[i]  = get_pipe_params()[index].dynamic_aibnr.auto_id[i];
    }
    ret = ss_mpi_aibnr_set_model_attr(is_wdr_mode, &aibnr_model_attr);
    check_scene_ret(ret);
    return TD_SUCCESS;
}

static td_void ot_scene_set_dynamic_aibnr_board(ot_vi_pipe vi_pipe, ot_aibnr_attr *aibnr_attr, td_u32 iso_level,
    td_u8 index)
{
    aibnr_attr->manual_attr.sfs = get_pipe_params()[index].dynamic_aibnr.sfs[iso_level];
    aibnr_attr->manual_attr.tfs = get_pipe_params()[index].dynamic_aibnr.tfs[iso_level];
    aibnr_attr->manual_attr.bright_denoise_strength =
        get_pipe_params()[index].dynamic_aibnr.bright_denoise_str[iso_level];
    aibnr_attr->manual_attr.dark_denoise_strength =
        get_pipe_params()[index].dynamic_aibnr.dark_denoise_str[iso_level];
    aibnr_attr->manual_attr.tnr.load_ref_en = get_pipe_params()[index].dynamic_aibnr.load_ref_en[iso_level];
    aibnr_attr->manual_attr.tnr.motion_gain = get_pipe_params()[index].dynamic_aibnr.motion_gain[iso_level];
    aibnr_attr->manual_attr.tnr.motion_thr = get_pipe_params()[index].dynamic_aibnr.motion_thr[iso_level];
    aibnr_attr->manual_attr.tnr.motion_noise_control =
        get_pipe_params()[index].dynamic_aibnr.motion_noise_control[iso_level];
    aibnr_attr->manual_attr.tnr.tf_speed_fine =
        get_pipe_params()[index].dynamic_aibnr.tf_speed_fine[iso_level];
    aibnr_attr->manual_attr.tnr.tf_speed_coarse =
        get_pipe_params()[index].dynamic_aibnr.tf_speed_coarse[iso_level];
}

static td_void ot_scene_set_dynamic_aibnr_interp(ot_vi_pipe vi_pipe, ot_aibnr_attr *aibnr_attr, td_u32 iso_level,
    td_u64 iso, td_u8 index)
{
    td_u32 *piso_level_thresh = get_pipe_params()[index].dynamic_aibnr.iso_level;
    aibnr_attr->manual_attr.sfs = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
    ...
    aibnr_attr->manual_attr.tfs = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
        get_pipe_params()[index].dynamic_aibnr.tfs[iso_level - 1], piso_level_thresh[iso_level],
        get_pipe_params()[index].dynamic_aibnr.tfs[iso_level]);
    aibnr_attr->manual_attr.bright_denoise_strength = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
        get_pipe_params()[index].dynamic_aibnr.bright_denoise_str[iso_level - 1],
        piso_level_thresh[iso_level], get_pipe_params()[index].dynamic_aibnr.bright_denoise_str[iso_level]);
    aibnr_attr->manual_attr.dark_denoise_strength = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
        get_pipe_params()[index].dynamic_aibnr.dark_denoise_str[iso_level - 1],
        piso_level_thresh[iso_level], get_pipe_params()[index].dynamic_aibnr.dark_denoise_str[iso_level]);
    aibnr_attr->manual_attr.tnr.load_ref_en = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
        get_pipe_params()[index].dynamic_aibnr.load_ref_en[iso_level - 1],
        piso_level_thresh[iso_level], get_pipe_params()[index].dynamic_aibnr.load_ref_en[iso_level]);
    aibnr_attr->manual_attr.tnr.motion_gain = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
        get_pipe_params()[index].dynamic_aibnr.motion_gain[iso_level - 1],
        piso_level_thresh[iso_level], get_pipe_params()[index].dynamic_aibnr.motion_gain[iso_level]);
    aibnr_attr->manual_attr.tnr.motion_thr = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
        get_pipe_params()[index].dynamic_aibnr.motion_thr[iso_level - 1],
        piso_level_thresh[iso_level], get_pipe_params()[index].dynamic_aibnr.motion_thr[iso_level]);
    aibnr_attr->manual_attr.tnr.motion_noise_control = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
        get_pipe_params()[index].dynamic_aibnr.motion_noise_control[iso_level - 1],
        piso_level_thresh[iso_level], get_pipe_params()[index].dynamic_aibnr.motion_noise_control[iso_level]);
    aibnr_attr->manual_attr.tnr.tf_speed_fine = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
        get_pipe_params()[index].dynamic_aibnr.tf_speed_fine[iso_level - 1],
        piso_level_thresh[iso_level], get_pipe_params()[index].dynamic_aibnr.tf_speed_fine[iso_level]);
    aibnr_attr->manual_attr.tnr.tf_speed_coarse = scene_interpulate(iso, piso_level_thresh[iso_level - 1],
        get_pipe_params()[index].dynamic_aibnr.tf_speed_coarse[iso_level - 1],
        piso_level_thresh[iso_level], get_pipe_params()[index].dynamic_aibnr.tf_speed_coarse[iso_level]);
}
...
td_s32 ot_scene_set_dynamic_aibnr(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    if ((iso >= aibnr_iso_thresh[1]) && (aibnr_chg_en == TD_TRUE)) {
        ...
        check_scene_ret(ret);
        iso_level = scene_get_level_ltoh_u32((td_u32)iso, iso_count, piso_level_thresh, OT_SCENE_ISO_STRENGTH_NUM);
        ot_scenecomm_expr_true_return(iso_level >= OT_SCENE_ISO_STRENGTH_NUM, TD_FAILURE);
        if (iso_level == 0) {
            ot_scene_set_dynamic_aibnr_board(vi_pipe, &aibnr_attr, iso_level, index);
        } else {
            ot_scene_set_dynamic_aibnr_interp(vi_pipe, &aibnr_attr, iso_level, iso, index);
        }
        aibnr_attr.blend = get_pipe_params()[index].dynamic_aibnr.blend_en;
        ret = ss_mpi_aibnr_set_attr(vi_pipe_id, &aibnr_attr);
        check_scene_ret(ret);
        ret = ot_scene_set_dynamic_aibnr_part(vi_pipe, index);
        check_scene_ret(ret);
    }
...
td_s32 ot_scene_set_dynamic_dehaze(ot_vi_pipe vi_pipe, td_u64 exposure, td_u64 last_exposure, td_u8 index)
{
    ...
    if ((exposure != last_exposure) || (last_ratio[vi_pipe] != actual_ratio)) {
        ...
        exp_level = scene_get_level_ltoh(exposure, get_pipe_params()[index].dynamic_dehaze.exp_thresh_cnt,
            get_pipe_params()[index].dynamic_dehaze.exp_thresh_ltoh, OT_SCENE_DEHAZE_EXPOSURE_MAX_COUNT);
...
static td_s32 ot_scene_set_dynamic_nr_part1(ot_scene_nr_para *nr_para, td_u8 index, td_u32 wdr_ratio)
{
    ...
    ratio_index = scene_get_level_ltoh_u32(wdr_ratio, ratio_count, level, OT_SCENE_DRC_RATIO_MAX_COUNT);
...
static td_s32 ot_scene_set_dynamic_dpc_part1(ot_isp_dp_dynamic_attr *dp_dynamic_attr, td_u8 index, td_u64 iso)
{
    ...
    td_u32 iso_level = (td_u32)scene_get_level_ltoh_u32((td_u32)iso, iso_count, iso_level_thresh, ISP_AUTO_ISO_DPC_NUM);
    ...
    if ((iso_level == (iso_count - 1)) || (iso_level == 0)) {
            ...
            dp_dynamic_attr->frame_dynamic[0].soft_thr =
                (td_s8)get_pipe_params()[index].dynamic_dpc.soft_thr[iso_level];
...
td_s32 ot_scene_set_dynamic_shading(ot_vi_pipe vi_pipe, td_u64 exposure, td_u64 last_exposure, td_u8 index)
{
    ...
    if (exposure != last_exposure) {
        ...
        exp_level = scene_get_level_ltoh(exposure, get_pipe_params()[index].dynamic_shading.exp_thresh_cnt,
            get_pipe_params()[index].dynamic_shading.exp_thresh_ltoh, OT_SCENE_SHADING_EXPOSURE_MAX_COUNT);
...
td_s32 ot_scene_set_dynamic_linear_ca(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    td_u32 ca_y_sat_lut_interp_lut[OT_ISP_CA_YRATIO_LUT_LENGTH];
    ...
    if ((iso_level == (iso_count - 1)) || (iso_level == 0)) {
        for (i = 0; i < OT_ISP_CA_YRATIO_LUT_LENGTH; i++) {
            ...
            ca_y_sat_lut_interp_lut[i] = get_pipe_params()[index].dynamic_ca.ca_y_sat_iso_lut[iso_level][i];
        }
    } else {
        td_u32 mid_iso = (td_u32)iso;
        td_u32 left_iso = iso_level_thresh[iso_level - 1];
...
static td_s32  ot_scene_set_dynamic_ca_part1(ot_isp_ca_attr *ca_attr, td_u8 index, td_u32 wdr_ratio, td_u32 iso)
{
    ...
    td_u32  iso_count = get_pipe_params()[index].dynamic_ca.iso_count;
    td_u32 *iso_level_thresh = get_pipe_params()[index].dynamic_ca.iso_level;
    td_u32 iso_level = scene_get_level_ltoh_u32((td_u32)iso, iso_count, iso_level_thresh, OT_SCENE_ISO_STRENGTH_NUM);
    ...
    td_u32 ratio_level = scene_get_level_ltoh_u32((td_u32)wdr_ratio, ratio_count, ratio_level_thresh,
        OT_SCENE_RATIO_STRENGTH_NUM);
    ot_scenecomm_expr_true_return(iso_level >= ISP_AUTO_ISO_CA_NUM, TD_FAILURE);
    if ((iso_level == (iso_count - 1)) || (iso_level == 0)) {
        for (i = 0; i < OT_ISP_CA_YRATIO_LUT_LENGTH; i++) {
            ca_y_ratio_iso_interp_lut[i] = get_pipe_params()[index].dynamic_ca.ca_y_ratio_iso_lut[iso_level][i];
        }
...
static td_s32  ot_scene_set_dynamic_color_sector_part1(ot_isp_color_sector_attr *cs_attr, td_u8 index, td_u32 iso)
{
    td_u32 i;
    td_u32 j;
    td_u32  iso_count = get_pipe_params()[index].dynamic_cs.iso_count;
    td_u32 *iso_level_thresh = get_pipe_params()[index].dynamic_cs.iso_level;
    td_u32 iso_level = (td_u32)scene_get_level_ltoh_u32((td_u32)iso, iso_count, iso_level_thresh,
        OT_SCENE_ISO_STRENGTH_NUM);
    ot_scenecomm_expr_true_return(iso_level >= ISP_AUTO_ISO_CS_NUM, TD_FAILURE);

    if ((iso_level == (iso_count - 1)) || (iso_level == 0)) {
        for (j = 0; j < OT_SCENE_CS_NUM; j++) {
...
{
    ot_scenecomm_expr_true_return(index >= OT_SCENE_PIPETYPE_NUM, TD_FAILURE);
    if ((get_pipe_params()[index].module_state.dynamic_color_sector != TD_TRUE) || (iso == last_iso)) {
        return TD_SUCCESS;
    }

    td_s32 ret;
    ot_isp_color_sector_attr cs_attr;
    check_scene_return_if_pause();
    ret = ss_mpi_isp_get_color_sector_attr(vi_pipe, &cs_attr);
    check_scene_ret(ret);
    ...
    check_scene_return_if_pause();
    ret = ss_mpi_isp_set_color_sector_attr(vi_pipe, &cs_attr);
    check_scene_ret(ret);
    return TD_SUCCESS;
}
...
td_s32 ot_scene_set_dynamic_blc(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    td_u32 iso_level = (td_u32)scene_get_level_ltoh_u32((td_u32)iso, iso_count, iso_level_thresh, ISP_AUTO_ISO_BLC_NUM);
    ...
    if ((iso_level == (iso_count - 1)) || (iso_level == 0)) {
        for (i = 0; i < OT_ISP_WDR_MAX_FRAME_NUM; i++) {
            isp_blc_attr.manual_attr.black_level[i][0] = get_pipe_params()[index].dynamic_blc.blc_r[iso_level];
            isp_blc_attr.manual_attr.black_level[i][1] = get_pipe_params()[index].dynamic_blc.blc_gr[iso_level];
            isp_blc_attr.manual_attr.black_level[i][2] = /* index2 */
                get_pipe_params()[index].dynamic_blc.blc_gb[iso_level];
            isp_blc_attr.manual_attr.black_level[i][3] = /* index3 */
        ...
        td_u32 right_iso = iso_level_thresh[iso_level];
        for (i = 0; i < OT_ISP_WDR_MAX_FRAME_NUM; i++) {
            isp_blc_attr.manual_attr.black_level[i][0] = scene_interpulate(mid_iso, left_iso,
                get_pipe_params()[index].dynamic_blc.blc_r[iso_level - 1], right_iso,
                get_pipe_params()[index].dynamic_blc.blc_r[iso_level]);
            isp_blc_attr.manual_attr.black_level[i][1] = scene_interpulate(mid_iso, left_iso,
                get_pipe_params()[index].dynamic_blc.blc_gr[iso_level - 1], right_iso,
                get_pipe_params()[index].dynamic_blc.blc_gr[iso_level]);
            isp_blc_attr.manual_attr.black_level[i][2] = scene_interpulate(mid_iso, left_iso, /* index2 */
                get_pipe_params()[index].dynamic_blc.blc_gb[iso_level - 1], right_iso,
                get_pipe_params()[index].dynamic_blc.blc_gb[iso_level]);
            isp_blc_attr.manual_attr.black_level[i][3] = scene_interpulate(mid_iso, left_iso, /* index3 */
...
{
    ot_scenecomm_expr_true_return(index >= OT_SCENE_PIPETYPE_NUM, TD_FAILURE);
    if ((get_pipe_params()[index].module_state.dynamic_csc != TD_TRUE) || (iso == last_iso)) {
        return TD_SUCCESS;
    }
    ...
    td_u32 iso_count = get_pipe_params()[index].dynamic_csc.iso_count;
    td_u32 *iso_level_thresh = get_pipe_params()[index].dynamic_csc.iso_level;
    td_u32 iso_level = (td_u32)scene_get_level_ltoh_u32((td_u32)iso, iso_count, iso_level_thresh,
        OT_SCENE_ISO_STRENGTH_NUM);
    ot_scenecomm_expr_true_return(iso_level >= OT_SCENE_ISO_STRENGTH_NUM, TD_FAILURE);
...
td_s32 ot_scene_set_dynamic_venc_bitrate(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    if (iso != last_iso) {
        ...
        td_u32 exp_level = scene_get_level_ltoh_u32((td_u32)iso,
            ...
            get_pipe_params()[index].dynamic_venc_bitrate.iso_thresh_ltoh, OT_SCENE_VENC_EXPOSURE_MAX_COUNT);
        ot_scenecomm_expr_true_return(exp_level >= OT_SCENE_SHADING_EXPOSURE_MAX_COUNT, TD_FAILURE);
...
td_s32 ot_scene_set_dynamic_video_gamma(ot_vi_pipe vi_pipe, td_u64 exposure, td_u64 last_exposure, td_u8 index)
{
    ...
    exp_level = scene_get_level_ltoh(exposure, get_pipe_params()[index].dynamic_gamma.total_num,
        get_pipe_params()[index].dynamic_gamma.exp_thresh_htol, OT_SCENE_GAMMA_EXPOSURE_MAX_COUNT);
...
td_s32 ot_scene_set_dynamic_false_color(ot_vi_pipe vi_pipe, td_u64 exposure, td_u64 last_exposure, td_u8 index)
{
    ...
    if (exposure != last_exposure) {
        ...
        exp_level = scene_get_level_ltoh_u32((td_u32)exposure, get_pipe_params()[index].dynamic_false_color.total_num,
            ...
            OT_SCENE_FALSECOLOR_EXPOSURE_MAX_COUNT);
...
td_s32 ot_scene_release_fpn(ot_vi_pipe vi_pipe, td_u8 index)
{
    ...
    if (g_fpn_enable[vi_pipe] == TD_TRUE) {
        ...
        ret = encode_scene_disable_fpn_correction(vi_pipe, &g_correction_cfg);
        check_scene_ret(ret);
...
static td_s32 ot_scene_dynamic_fpn_decide(ot_vi_pipe vi_pipe, td_u32 *iso_tmp, td_u32 *last_iso_tmp,
    td_bool *fpn_enable, td_u8 index)
{
    ...
    if (sns_iso >= iso_level_thresh[iso_count - 1]) {
        ...
        *iso_tmp = sns_iso;
        ...
        for (i = 0; (i < iso_count - 1) && i < OT_SCENE_ISO_STRENGTH_NUM; i++) {
...
td_s32 ot_scene_set_dynamic_fpn(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    td_u32 iso_count = get_pipe_params()[index].dynamic_fpn.iso_count;
    td_u32 *iso_level_thresh = get_pipe_params()[index].dynamic_fpn.iso_thresh;
    //encode_scene_fpn_offset_cfg  scene_fpn_offset_cfg;
    ...
    if (last_iso_tmp[vi_pipe] != iso_tmp && (g_fpn_enable[vi_pipe] == TD_TRUE)) {
        ...
        ret = encode_scene_enable_fpn_correction(vi_pipe, &g_correction_cfg);
        check_scene_ret(ret);
...
td_s32 ot_scene_set_dynamic_3dnr(ot_vi_pipe vi_pipe, td_u32 iso, td_u8 index)
{
    ...
    iso_level = scene_get_level_ltoh_u32(iso, count, thresh, OT_SCENE_3DNR_MAX_COUNT);
    if (iso_level == 0) {
    ...
    check_scene_return_if_pause();
    td_s32 ret = scene_set_3dnr(vi_pipe, &nrx_attr, index);
    check_scene_ret(ret);
    return TD_SUCCESS;
}
