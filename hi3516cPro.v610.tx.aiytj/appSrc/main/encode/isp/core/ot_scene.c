...
static td_void scene_get_last_exposure(scene_thread_type thread_type, td_u32 i, td_u64 *last_exposure)
{
    switch (thread_type) {
...
static td_void scene_get_last_iso(scene_thread_type thread_type, td_u32 i, td_u32 *last_iso)
{
    switch (thread_type) {
        case SCENE_THREAD_TYPE_NORMAL:
...
static td_void proc_normal_auto(td_void)
{
    td_s32 ret;
    td_u32 i;
    static td_bool first_loop = TD_TRUE;
    static int old = TD_FALSE, tick =0;
    int new = get_facial_convergence_status();
    if (new) {
        if (new != old ) {
            tick = 10;      // 10*200ms
        } else {
            if (tick < 0) {
                ...
                tick--;
                printf("convergence wait,tick=%d\n",tick);
            }
        }
    }
    old = new;
...
static td_void *scene_normal_auto_thread(td_void *arg)
{
    td_s32 ret;
    td_u32 i, j;
    prctl(PR_SET_NAME, (unsigned long)(uintptr_t)"SCENE_Normal", 0, 0, 0);
...
static td_void proc_luminance_auto(td_void)
{
    td_s32 ret;
    td_u32 i;
    static td_bool first_loop = TD_TRUE;
...
static td_void *scene_luminance_auto_thread(td_void *arg)
{
    prctl(PR_SET_NAME, (unsigned long)(uintptr_t)"SCENE_Luminance", 0, 0, 0);
...
static td_void proc_not_linear_auto(td_void)
{
    td_s32 ret;
    td_u32 i;
...
static td_void *scene_not_linear_auto_thread(td_void *arg)
{
    prctl(PR_SET_NAME, (unsigned long)(uintptr_t)"SCENE_NotLinear", 0, 0, 0);
...
static td_void proc_venc_auto(td_void)
{
    td_s32 ret;
    ...
    for (td_u32 i = 0; i < g_scene_state.main_pipe_num; i++) {
        ...
        ret = ot_scene_set_rc_param(vi_pipe, (td_u8)index);
        ot_scenecomm_check(ret, OT_SCENE_EINTER);
    }
}

static void *scene_venc_auto_thread(td_void *arg)
{
    prctl(PR_SET_NAME, (unsigned long)(uintptr_t)"SCENE_VencAuto", 0, 0, 0);
...
td_s32 ot_scene_run_once(td_void)
{
    if (g_scene_state.pause == TD_FALSE) {
        scene_loge("can't call scene_run_once() when sceneauto is running, call ot_scene_pause(TRUE) first!\n");
        return TD_FAILURE;
    }
    set_run_once(TD_TRUE);
    proc_normal_auto();
    proc_luminance_auto();
    proc_not_linear_auto();
    proc_venc_auto();
    set_run_once(TD_FALSE);
    return TD_SUCCESS;
}
...
static td_s32 scene_start_auto_thread(td_void)
{
    ...
    if ((g_scene_state.thread_not_linear.thread_flag == TD_FALSE) &&
        ...
        ret = pthread_create(&g_scene_state.thread_not_linear.thread, &(g_scene_state.thread_not_linear_attr),
