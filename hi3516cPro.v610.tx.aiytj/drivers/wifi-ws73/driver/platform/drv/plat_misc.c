...
OAL_STATIC osal_u32  uapi_ccpriv_get_version(osal_s8 *param)
{
    ...
    oal_print_production_info("SDK_Version:       1.10.111\r\n");
...
#define CONVERT_TO_DEC 10
OAL_STATIC osal_u32  uapi_ccpriv_cfg_local_diag(osal_s8 *param)
{
    osal_u32 arg_ret;
    osal_u32 off_set;
    osal_u8 arg1[CONVERT_TO_DEC] = {0};
    osal_u8 arg2[CONVERT_TO_DEC] = {0};
    osal_u8 arg3[CCPRIV_CMD_NAME_MAX_LEN] = {0};
    osal_char *path;
    osal_u8 enable;
    osal_u8 level;
    ...
    enable = (osal_u8)osal_strtol(arg1, OAL_PTR_NULL, CONVERT_TO_DEC);
    ...
    level = (osal_u8)osal_strtol(arg2, OAL_PTR_NULL, CONVERT_TO_DEC);
    if (level >= OAM_LOG_LEVEL_MAX) {
        oal_print_err("uapi_ccpriv_cfg_local_diag, err level[%d] range[0-7]\r\n", level);
        return OAL_FAIL;
    }

    param += off_set;
    arg_ret = plat_get_cmd_one_arg(param, arg3, sizeof(arg3), &off_set);
    if (arg_ret != OAL_SUCC) {
        oal_print_err("plat_get_cmd_one_arg return err_code [%d]!\r\n", arg_ret);
        return OAL_FAIL;
    }
    if (strlen(arg3) >= OAM_FILE_MAX_NAME_LEN - 1) {
        oal_print_err("uapi_ccpriv_cfg_local_diag, err path'len max [%d]!\r\n", OAM_FILE_MAX_NAME_LEN);
        return OAL_FAIL;
    }
    path = (osal_char *)arg3;

    oal_print_production_info("uapi_ccpriv_cfg_local_diag, open:%hhu, level:%hhu[err:3, warn:4,info:6], path:%s \r\n",
        enable, level, path);
    return zdiag_local_log_set_cfg(enable, level, path);
}
#endif
...
#if defined(CONFIG_DIAG_SUPPORT_LOCAL_LOG)
            {"diag_log", uapi_ccpriv_cfg_local_diag},
#endif
};
