...
ext_errno diag_sync_cfg_cmd_to_dev(td_u32 level)
{
    ...
    td_u8 msp_diag_data[ZDIAG_MSG_REQ_DATA_LEN];
...
ext_errno diag_sync_con_cmd_to_dev(td_bool connect)
{
    diag_option option = DIAG_OPTION_INIT_VAL;
    td_u8 msp_diag_data[ZDIAG_MSG_REQ_DATA_LEN] = {0};
    diag_cmd_msg_cfg_req_stru *msg_cfg_req = (diag_cmd_msg_cfg_req_stru *)msp_diag_data; // outer msg: msp_diag_head
    td_u16 con = (connect == TD_FALSE) ? DIAG_CMD_HOST_DISCONNECT : DIAG_CMD_HOST_CONNECT;

    if (!wlan_pm_is_poweron_etc()) {
        return EXT_ERR_SUCCESS;
    }
    return diag_hcc_sync_cmd_to_dev(con, msg_cfg_req, 0, &option);
}
#endif
#endif
