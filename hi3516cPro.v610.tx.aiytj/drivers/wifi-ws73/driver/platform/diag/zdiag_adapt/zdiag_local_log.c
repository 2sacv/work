...
static td_u8 g_local_log_dbg_level = PLAT_LOG_ERR;
static td_bool g_local_log_inited = TD_FALSE;
...
static void *diag_local_log_file_alter(td_u16 file_pos)
{
    ...
    if (file_path[0] == '\0') {
        ...
        return osal_klib_fopen(file_path, O_TRUNC | O_CREAT | O_RDWR, KLIB_MODE_T); // clear/create file
    }
}
...
{
    return osal_klib_stat(file_name);
}
...
static osal_s32 zdiag_local_log_dir_init(void)
{
    osal_s32 ret = 0;
    char *dir_name = NULL;
    td_u8 file_num;
    ...
    if (zdiag_local_log_get_stat(dir_name) != 0) {
...
static td_s32 zdiag_local_log_cfg_init(void)
{
    ...
    osal_s32 ini_value = DIAG_LOG_DISABLE;
    ...
    if (ini_value != DIAG_LOG_ENABLE) {
        oam_error("ini local_log disable\n");
        return EXT_ERR_DIAG_NOT_SUPPORT;
    }
...
td_u32 zdiag_local_log_set_cfg(td_u8 enable, td_u8 level, td_char *path)
{
    td_u8 mode = (enable == 0) ? DIAG_LOG_TO_HSO : DIAG_LOG_TO_FILE;
    td_bool connect = (enable == 0) ? TD_FALSE : TD_TRUE;
    if (level >= OAM_LOG_LEVEL_MAX || path == TD_NULL) {
        return EXT_ERR_DIAG_INVALID_PARAMETER;
    }

    // step1: pause log out
    zdiag_set_enable(TD_FALSE, ACORE_HSO_ADDR);
    diag_sync_con_cmd_to_dev(TD_FALSE);
    if (!g_local_log_inited) {
        if (zdiag_local_log_cfg_init() != EXT_SUCCESS) {
            return EXT_ERR_DIAG_NO_INITILIZATION;
        }
        g_local_log_inited = TD_TRUE;
    }

    // step2: set path
    if (osal_strcmp(g_file_mng.file_dir, path) != 0) {
        strcpy_s(g_file_mng.file_dir, OAM_FILE_MAX_NAME_LEN, path);
        if (zdiag_local_log_dir_init() != EXT_SUCCESS) {
            return EXT_FAILURE;
        }
    }

    // step3: set flag
    diag_hcc_conn_state_set(connect);
    zdiag_log_mode_set(mode);
    diag_dis_cmd_support_or_not(!connect);

    // step4: set level
    if (connect) {
        diag_sync_cfg_cmd_to_dev((td_u32)level);
        diag_enable_level_switch((td_u32)level);
    }
    return EXT_SUCCESS;
}
